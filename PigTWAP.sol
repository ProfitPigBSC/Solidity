...
contract TWAP {
    ...
    constructor(IUniswapV2Pair _pair, address _auth) public {
        pair = _pair;
        auth = _auth;
        token0 = _pair.token0();
        token1 = _pair.token1();
    ...
    function _checkCaller() internal view {
        require(msg.sender == auth, "Caller not authorised");
    }

    function update() external onlyAuth {
        (
    ...
    function consult(address token) external view returns (uint amountOut) {
        ...
        if (token == token0) {
            ...
            amountOut = price1Average.mul(multiplier).decode144();
        }
    }
